...
check_and_fix_env() {
    if ! command -v jq &> /dev/null || ! command -v curl &> /dev/null; then
        echo -e "${YELLOW}检测到必要工具缺失，正在尝试安装...${NC}"
...
# ================= 核心功能：安装与系统优化 =================
install_realm() {
    echo -e "${CYAN}--- 开始安装 Realm 并进行中转专用优化 ---${NC}"
    
    echo -e "${YELLOW}1. 通过国产镜像下载最新版 Realm 核心...${NC}"
    # 使用 ghproxy.net 加速镜像
    curl -L -o realm.tar.gz "https://ghproxy.net/https://github.com/zhboner/realm/releases/latest/download/realm-x86_64-unknown-linux-gnu.tar.gz" >/dev/null 2>&1
    
    if [ ! -f "realm.tar.gz" ]; then
        echo -e "${RED}❌ 下载失败，请检查国内网络连接！${NC}"
        return
    fi
    ...
    echo -e "${YELLOW}2. 初始化基础配置 (开启 TCP No Delay)...${NC}"
    mkdir -p $CONFIG_DIR
...
EOCC
    ...
    echo -e "${YELLOW}3. 写入系统守护进程 (解除 100W+ 文件连接限制)...${NC}"
...
EOT
    ...
    echo -e "${GREEN}✅ Realm 安装及国内中转优化完毕！${NC}"
    sleep 2
}
...
add_rule() {
    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${RED}尚未安装 Realm，请先选 1 安装！${NC}"; sleep 2; return
    fi
    echo -e "${CYAN}--- 添加新转发规则 ---${NC}"
    read -p "请输入 [本地监听端口]: " LOCAL_PORT
    read -p "请输入 [落地机目标IP]: " REMOTE_IP
    read -p "请输入 [落地机目标端口]: " REMOTE_PORT
    ...
    systemctl restart realm
    echo -e "${GREEN}✅ 规则已添加: 本机:$LOCAL_PORT -> 目标:$REMOTE_IP:$REMOTE_PORT${NC}"
    sleep 2
}
...
delete_rule() {
    ...
    if [[ "$IDX" =~ ^[0-9]+$ ]] && [ "$IDX" -ge 0 ] && [ "$IDX" -lt "$COUNT" ]; then
        ...
        systemctl restart realm
        echo -e "${GREEN}编号 $IDX 的规则已删除并重启生效！${NC}"
    else
        echo -e "${RED}无效的编号！${NC}"
    fi
    sleep 2
...
# ================= 主循环逻辑 =================
check_and_fix_env # 脚本运行即确保环境就绪
while true; do
    clear
    echo -e "${GREEN}=================================================${NC}"
    echo -e "${GREEN}      Realm 守护脚本 (中转加速 V3.3)      ${NC}"
    echo -e "${GREEN}=================================================${NC}"
    ...
    echo -e ""
    echo -e "  ${YELLOW}1.${NC} 安装/更新 Realm (含国内链路优化)"
